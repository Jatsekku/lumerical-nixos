...
let
  displayToolsVersions = ''
    ...
    DOCKER_VER=$(docker -v | head -n1 | grep -oP '(?<=version )\d+\.\d+\.\d+')
    echo "Docker: $DOCKER_VER"

    # Get and display hadolint version
    HADOLINT_VER=$(hadolint -v | grep -oP '\d+\.\d+\.\d+')
    echo "Hadolint: $HADOLINT_VER"
  ...
  displayGreetings = ''
    echo "Lumerical-dockerized environment"
  '';
in
{
